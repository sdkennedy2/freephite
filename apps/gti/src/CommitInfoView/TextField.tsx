...
export function CommitInfoTextField({
  ...
  useEffect(() => {
    if (ref.current && autoFocus) {
      const inner = getInnerTextareaForVSCodeTextArea(
  ...
  const [typeaheadSuggestions, setTypeaheadSuggestions] =
    useState<TypeaheadSuggestions>(undefined);
  ...
  const onInput = (event: { target: EventTarget | null }) => {
    ...
    if (
      typeaheadSuggestions?.type !== "success" ||
      typeaheadSuggestions.values.length === 0
    ) {
      setTypeaheadSuggestions({ type: "loading" });
    }
    void fetchNewSuggestions(typeaheadKind, newValue).then(
      ({ values, fetchStartTimestamp }) => {
  ...
  return (
    <div
      ...
      onKeyDown={(event) => {
        ...
        const values = (
          typeaheadSuggestions as TypeaheadSuggestions & { type: "success" }
        )?.values;
        if (values == null) {
          return;
        }
          ...
          <div className="typeahead-suggestions tooltip tooltip-bottom">
            ...
            {typeaheadSuggestions?.type === "loading" ? (
              ...
              typeaheadSuggestions?.values.map((suggestion, index) => (
                ...
                >
                  {suggestion.image && (
                    <ImageWithFallback src={suggestion.image} />
                  )}
