...
export function emptyCommitMessageFields(
  schema: Array<FieldConfig>
): CommitMessageFields {
  return Object.fromEntries(
    schema.map((config) => [config.key, config.type === "field" ? [] : ""])
  );
}
 ...
 */
export function noFieldsBeingEdited(
  schema: Array<FieldConfig>
): FieldsBeingEdited {
 ...
 */
export function allFieldsBeingEdited(
  schema: Array<FieldConfig>
): FieldsBeingEdited {
...
export function findFieldsBeingEdited(
  ...
  return Object.fromEntries(
    schema.map((config) => [
      ...
      config.type === "field"
        ? !arraysEqual(
            a[config.key] as Array<string>,
            b[config.key] as Array<string>
          )
  ...
  return schema
    ...
    .map(
      (config) =>
        ...
        (config.key === "Title" || config.key === "Description"
          ? ""
          : config.key + ": ") +
        (config.type === "field"
...
export function parseCommitMessageFields(
  ...
  const result = Object.fromEntries(
    schema.map((config) => {
      ...
      return [
        config.key,
        config.type === "field" ? commaSeparated(found) : found,
      ];
    })
  );
 ...
 */
export const commitMessageFieldsSchema = observable.box<Array<FieldConfig>>(
  getDefaultCommitMessageSchema()
);
serverAPI.onCwdChanged(() =>
  commitMessageFieldsSchema.set(getDefaultCommitMessageSchema())
);

export function getDefaultCommitMessageSchema() {
  return OSSDefaultFieldSchema;
}
