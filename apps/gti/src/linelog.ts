...
class LineLog extends SelfUpdate<LineLogRecord> {
  ...
  editChunk(
    ...
    const aLinesInfo: [LineInfo[], true] | [Readonly<LineInfo[]>, false] =
      aLinesMutable ? [aLinesCache, true] : [this.checkOutLines(aRev), false];
  ...
  public recordText(text: string, rev: Rev | null = null): LineLog {
    ...
    const [aRev, bRev] =
      rev != null ? [rev, rev] : [this.maxRev, this.maxRev + 1];
    const b = text;
...
function diffLines(
  ...
  while (
    ...
    aList[commonPrefixLen] === bList[commonPrefixLen]
  ) {
    commonPrefixLen += 1;
  }
  while (
