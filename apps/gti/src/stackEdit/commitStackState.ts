...
import type {
  ...
  Hash,
  RepoPath,
} from "@withgraphite/gti-shared/types/common";
import type {
  ExportStack,
  ExportFile,
  ImportStack,
...
import { cached } from "@withgraphite/gti-shared/LRU";
import { SelfUpdate } from "@withgraphite/gti-shared/immutableExt";
import { generatorContains, unwrap, zip } from "@withgraphite/gti-shared/utils";
...
export class CommitStackState extends SelfUpdate<CommitStackRecord> {
  ...
  set<K extends keyof CommitStackProps>(
    key: K,
    value: CommitStackProps[K]
  ): CommitStackState {
  ...
  mutableRevs(): Rev[] {
    return [
      ...this.stack.filter((c) => c.immutableKind !== "hash").map((c) => c.rev),
    ];
  }
    ...
    while (true) {
      ...
      if (commit != null) {
        ...
        commit.parents.forEach((parentRev) => {
          assert(
            parentRev < rev,
            "parent rev must < child to prevent infinite loop in log()"
          );
  ...
  *logFile(
  ...
  calculateImportStack(opts?: {
    ...
    const gotoRev: Rev | undefined =
      typeof opts?.goto === "string"
        ? this.originalStack.at(-1)?.node == opts.goto
          ? this.stack.last()?.rev
          : this.stack.findLastKey((c) =>
              c.originalNodes.has(opts.goto as string)
            )
        : opts?.goto;
    ...
    const revToMarkOrHash = (rev: Rev): Mark | Hash => {
      if (changedRevs.has(rev)) {
        ...
        assert(
          nodes.size === 1,
          "unchanged commits should have exactly 1 nodes"
        );
    ...
    const actions: ImportAction[] = changedCommits.map((commit) => {
      assert(
        commit.immutableKind !== "hash",
        "immutable commits should not be changed"
      );
      const newFiles: { [path: RepoPath]: ExportFile | null } =
        Object.fromEntries(
  ...
  buildFileStacks(): CommitStackState {
    ...
    const processFile = (
      ...
      if (isUtf8(file)) {
        ...
        if (prevRev >= 0) {
          ...
          const prev = commitToFile.get(
            CommitIdx({ rev: prevRev, path: prevPath })
          );
          if (prev) {
    ...
    state.stack.forEach((commit, rev) => {
      ...
      const priorityFiles: [number, RepoPath, FileState][] = [
        ...files.entries(),
      ].map(([path, file]) => {
        const priority = isRename(commit, path)
          ? 0
          : file.copyFrom == null
          ? 1
          : 2;
      ...
      .map((fileStack, fileIdx) => {
        return fileStack
          ...
          .map((fileRev) => {
            ...
            assert(
              value != null,
              "fileToCommit should have all file stack revs"
            );
  ...
  getUtf8Data(file: FileState): string {
    ...
    if (file.data instanceof FileIdx) {
      return unwrap(this.fileStacks.get(file.data.fileIdx)).getRev(
        file.data.fileRev
      );
    } else {
  ...
  isStackLinear(): boolean {
    return this.stack.every(
      (commit, rev) =>
        rev === 0 ||
        (commit.parents.size === 1 && commit.parents.first() === rev - 1)
    );
  }
  ...
  calculateDepMap(): Readonly<Map<Rev, Set<Rev>>> {
    ...
    const depMap = new Map<Rev, Set<Rev>>(
      state.stack.map((c) => [c.rev, new Set()])
    );

    const fileIdxRevToCommitRev = (
      fileIdx: FileStackIndex,
      fileRev: Rev
    ): Rev => unwrap(state.fileToCommit.get(FileIdx({ fileIdx, fileRev }))).rev;
    ...
    state.stack.forEach((commit) => {
      ...
      commit.files.forEach((file, path) => {
        const [prevRev, prevPath, prevFile] = state.parentFile(
          commit.rev,
          path,
          true
        );
        if (
          prevRev >= 0 &&
          (isAbsent(prevFile) !== isAbsent(file) || prevPath !== path)
        ) {
          set.add(prevRev);
        }
      });
  ...
  foldDown(rev: Rev) {
    ...
    const newFiles = commit.files.map((origFile, path) => {
      ...
      if (
        copyFrom != null &&
        isAbsent(this.parentFile(parentRev, file.copyFrom ?? path)[2])
      ) {
      ...
      if (
        this.isEqualFile(
          this.parentFile(parentRev, path, false /* [1] */)[2],
          file
        )
      ) {
  ...
  canDrop(rev: Rev): boolean {
    ...
    for (const [currentRev, dependentRevs] of depMap.entries()) {
      if (
        dependentRevs.has(rev) &&
        generatorContains(this.log(currentRev), rev)
      ) {
        return false;
      }
    }
  ...
  drop(rev: Rev): CommitStackState {
    ...
    commit.files.forEach((file, path) => {
      const fileIdxRev: FileIdx | undefined = state.commitToFile.get(
        CommitIdx({ rev, path })
      );
      if (fileIdxRev != null) {
        ...
        const newFileStack = fileStack.remapRevs(
          new Map([[fileRev, unusedFileRev]])
        );
  ...
  canReorder(order: Rev[]): boolean {
    ...
    if (
      state.stack.some(
        (commit, rev) => commit.immutableKind === "hash" && order[rev] !== rev
      )
    ) {
      return false;
    }

    const map = new Map<Rev, Rev>(
      order.map((fromRev, toRev) => [fromRev, toRev])
    );
    // Check dependencies.
  ...
  reorder(order: Rev[]): CommitStackState {
    const commitRevMap = new Map<Rev, Rev>(
      order.map((fromRev, toRev) => [fromRev, toRev])
    );
    ...
    const newFileStacks = state.fileStacks.map((origFileStack, fileIdx) => {
      ...
      const mappedCommitRevs: Rev[] = commitRevs.map(
        (rev) => commitRevMap.get(rev) ?? rev
      );
      ...
      return new FileStackState(
        fileRevs.map((fileRev) => fileStack.getRev(toRevs[fileRev]))
      );
    });
...
function getBottomFilesFromExportStack(
  stack: Readonly<ExportStack>
): Map<RepoPath, FileState> {
...
function convertExportFileToFileState(file: ExportFile | null): FileState {
  ...
  return FileState({
    data:
      file.data != null
        ? file.data
        : Base85({ dataBase85: unwrap(file.dataBase85) }),
    copyFrom: file.copyFrom,
    flags: file.flags,
  });
}

function getCommitStatesFromExportStack(
  stack: Readonly<ExportStack>
): List<CommitState> {
  ...
  const nodeToRevMap: Map<Hash, Rev> = new Map(
    revs.map((rev) => [stack[rev].node, rev])
  );
  const nodeToRev = (node: Hash): Rev => {
    const rev = nodeToRevMap.get(node);
    if (rev == null) {
      throw new Error(
        `Rev ${rev} should be known ${JSON.stringify(
          nodeToRevMap
...
function checkStackSingleRoot(stack: Readonly<ExportStack>) {
  const rootNodes = stack.filter(
    (commit) => (commit.parents ?? []).length === 0
  );
  if (rootNodes.length > 1) {
    throw new Error(
      `Multiple roots ${JSON.stringify(
        rootNodes.map((c) => c.node)
      )} is not supported`
    );
  }
}
...
function checkStackParents(stack: Readonly<ExportStack>) {
    ...
    if (parents.length > 0) {
      if (!commit.requested) {
        throw new Error(
          `Requested commit ${
            commit.node
          } should not have parents ${JSON.stringify(
            parents
          )} (bug in debugexportstack?)`
        );
      }
      parents.forEach((parentNode) => {
        if (!knownNodes.has(parentNode)) {
          throw new Error(
            `Parent commit ${parentNode} is not exported (bug in debugexportstack?)`
          );
        }
      });
    }
  ...
  if (knownNodes.size != stack.length) {
    throw new Error(
      "Commit stack has duplicated nodes (bug in debugexportstack?)"
    );
  }
}
...
const FileState = Record<FileStateProps>({
  data: "",
  copyFrom: undefined,
  flags: "",
});
