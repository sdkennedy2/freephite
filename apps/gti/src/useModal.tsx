...
const modalState = observable.box<ModalState<unknown | string> | null>(null, {
  deep: false,
});
...
export const ModalContainer = observer(() => {
  ...
  const dismiss = () => {
    if (modal?.visible) {
      ...
      modalState.set({ ...modal, visible: false });
    }
  };
    ...
    content = (
      <>
        ...
        <div className="use-modal-buttons">
          {config.buttons.map(
            (button: string | ButtonConfig, index: number) => {
              const label = typeof button === "object" ? button.label : button;
              const isPrimary =
                typeof button === "object" && button.primary != null;
              return (
                <VSCodeButton
    ...
    >
      <div id="use-modal-title">
        {modal.config.icon != null ? (
          <Icon icon={modal.config.icon} size="M" />
        ) : null}
