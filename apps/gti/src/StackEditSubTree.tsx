...
export function StackEditSubTree(): React.ReactElement {
  ...
  const draggingHintText: string | null =
    reorderState.draggingRevs.size > 1
      ? "Dependent commits are moved together"
      : null;
      ...
      if (isDragging) {
        ...
        if (commitStack.canReorder(order) && !currentReorderState.isNoop()) {
          ...
          stackEdit.push(newStackState, {
            ...
            commit: unwrap(
              commitStack.stack.get(currentReorderState.draggingRev)
            ),
          });
        ...
        <div className="stack-edit-dragging" ref={draggingDivRef}>
          ...
          {draggingHintText && (
            <div className="stack-edit-dragging-hint-container">
              <span className="stack-edit-dragging-hint-text tooltip">
                {draggingHintText}
              </span>
            </div>
          )}
...
export function StackEditCommit({
  ...
  const handleMoveUp = () => {
    stackEdit.push(state.reorder(reorderedRevs(state, rev)), {
      name: "move",
      offset: 1,
      commit,
    });
  ...
  const buttons = (
    <div className="stack-edit-button-group">
      ...
      <Tooltip
        title={
          canFold
            ...
            : "Can not fold with parent if this commit is at the bottom"
        }
      >
        <VSCodeButton
          disabled={!canFold}
      ...
      <Tooltip
        title={
          canDrop
            ? "Drop the commit in the stack"
            : "Cannot drop this commit because it has dependencies"
        }
      >
        <VSCodeButton
          disabled={!canDrop}
      ...
      <div className="commit-rows">
        ...
        <div
          className={`commit-details${
            isReorderPreview ? " commit-reorder-preview" : ""
          }`}
        >
...
function calculateReorderOffset(
  ...
  container.querySelectorAll(".commit").forEach((element) => {
    ...
    const commitY =
      unwrap(commitDiv.offsetParent).getBoundingClientRect().y +
      commitDiv.offsetTop;
    if (commitY > y) {
      belowCount += 1;
    }
...
export function UndoDescription({
  ...
  if (op.name === "move") {
    ...
    if (offset === 1) {
      return (
        <>
          moving up <CommitTitle commit={commit} />
        </>
      );
    } else if (offset === -1) {
      return (
        <>
          moving down <CommitTitle commit={commit} />
        </>
      );
    } else if (offset > 0) {
      if (depCount > 0) {
        return (
          <>
            moving up <CommitTitle commit={commit} /> and {depCount} more
          </>
        );
      } else {
        return (
          <>
            moving up <CommitTitle commit={commit} /> by{" "}
            {Math.abs(offset).toString()} commits
          </>
        );
      }
    } else {
      if (depCount > 0) {
        return (
          <>
            moving down <CommitTitle commit={commit} /> and {depCount} more
          </>
        );
      } else {
        return (
          <>
            moving down <CommitTitle commit={commit} /> by{" "}
            {Math.abs(offset).toString()} commits
          </>
        );
      }
    ...
    return (
      <>
        folding down <CommitTitle commit={op.commit} />
      </>
    );
  } else if (op.name === "drop") {
    return (
      <>
        dropping <CommitTitle commit={op.commit} />
      </>
    );
...
function CommitTitle({ commit }: { commit: CommitState }): React.ReactElement {
  return (
    <span className="commit-title">{commit.text.split("\n", 1).at(0)}</span>
  );
}
