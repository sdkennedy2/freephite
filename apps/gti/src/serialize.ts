...
export function serialize(arg: Serializable): Serialized {
  ...
  if (arg instanceof Map) {
    return {
      __rpcType: "Map",
      data: Array.from(arg.entries()).map(([key, val]) => [
        serialize(key),
        serialize(val),
      ]),
    } as CustomSerialized;
    ...
    return {
      __rpcType: "Set",
      data: Array.from(arg.values()).map(serialize),
    } as CustomSerialized;
  } else if (arg instanceof Error) {
    return {
      __rpcType: "Error",
      data: { message: arg.message, stack: arg.stack },
    } as CustomSerialized;
    ...
    const newObj: CustomSerialized & { __rpcType: "object" } = {
      __rpcType: "object",
    };
