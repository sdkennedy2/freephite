...
export interface MessageBus {
  ...
  onChangeStatus(
    handler: (newStatus: MessageBusStatus) => void | Promise<void>
  ): Disposable;
...
class VSCodeMessageBus {
  ...
  onMessage(
    handler: (event: MessageEvent<string>) => void | Promise<void>
  ): Disposable {
  ...
  onChangeStatus(
    handler: (newStatus: MessageBusStatus) => unknown
  ): Disposable {
...
export class LocalWebSocketEventBus {
  ...
  private exponentialReconnectDelay =
    LocalWebSocketEventBus.DEFAULT_RECONNECT_CHECK_TIME_MS;
  private queuedMessages: Array<string | ArrayBuffer> = [];

  private handlers: Array<
    (event: MessageEvent<string>) => void | Promise<void>
  > = [];
  private statusChangeHandlers: Array<
    (newStatus: MessageBusStatus) => unknown
  > = [];
  ...
  private startConnection(): WebSocket {
    ...
    const platformName = window.gtiPlatform?.platformName;
    if (platformName) {
  ...
  private scheduleReconnect() {
    ...
    logger.info(
      `websocket connecion closed. Retrying in ${this.exponentialReconnectDelay}ms`
    );
    setTimeout(() => {
  ...
  onChangeStatus(
    ...
    void handler(this.status); // seed with current status
    const dispose = () => {
