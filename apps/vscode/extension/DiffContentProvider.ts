...
import type { Disposable } from "@withgraphite/gti/src/types";
 ...
 *
 * GraphiteDiffContentProvider uses a repository to provide the "original"
 * file content for a comparison using `sl cat`.
 */
export class GraphiteDiffContentProvider
  implements vscode.TextDocumentContentProvider
{
  ...
  private activeUrisByRepo: Map<
    Repository | "unknown",
    Set<string /* serialized GraphiteDiffEncodedUri */>
  > = new Map();
  ...
  constructor(private logger: Logger) {
    ...
    repositoryCache.onChangeActiveRepos((activeRepos) => {
      ...
      if (unownedComparisons) {
        ...
        for (const encoded of unownedComparisons.values()) {
          ...
          const { fsPath } = decodeGraphiteDiffUri(encodedUri).originalUri;
          for (const root of knownRoots) {
            if (
              fsPath === root ||
              fsPath.startsWith(ensureTrailingPathSep(root))
            ) {
    ...
    this.disposables.push(
      ...
      vscode.workspace.onDidCloseTextDocument((e) => {
        if (e.uri.scheme === GRAPHITE_DIFF_PROVIDER_SCHEME) {
  ...
  async provideTextDocumentContent(
    ...
    const data = decodeGraphiteDiffUri(encodedUri);
    ...
    const activeUrisSet =
      this.activeUrisByRepo.get(repo ?? "unknown") ?? new Set();
    ...
    const cachedFileContent =
      this.fileContentsByEncodedUri.get(encodedUriString);
...
export function registerGraphiteDiffContentProvider(
  logger: Logger
): vscode.Disposable {
  return vscode.workspace.registerTextDocumentContentProvider(
    GRAPHITE_DIFF_PROVIDER_SCHEME,
    new GraphiteDiffContentProvider(logger)
  );
}

export const GRAPHITE_DIFF_PROVIDER_SCHEME = "graphite-diff";
/**
 * {@link vscode.Uri} with scheme of {@link GRAPHITE_DIFF_PROVIDER_SCHEME}
 */
type GraphiteDiffEncodedUri = vscode.Uri;

type GraphiteURIEncodedData = {
  comparison: Comparison;
};

/**
 * Encode a normal file's URI plus a comparison revset
 * to get the custom URI which {@link GraphiteDiffContentProvider} knows how to provide content for
 * that file at that revset.
 * Decoded by {@link decodeGraphiteDiffUri}.
 */
export function encodeGraphiteDiffUri(
  uri: vscode.Uri,
  comparison: Comparison
): GraphiteDiffEncodedUri {
  if (uri.scheme !== "file") {
    throw new Error(
      "encoding non-file:// uris as graphite diff uris is not supported"
    );
  }
  return uri.with({
    scheme: GRAPHITE_DIFF_PROVIDER_SCHEME,
    query: JSON.stringify({
      comparison,
    } as GraphiteURIEncodedData),
  });
}

/**
 * Decode a custom URI which was encoded by  {@link encodeGraphiteDiffUri},
 * to get the original file URI back.
 */
export function decodeGraphiteDiffUri(uri: GraphiteDiffEncodedUri): {
  originalUri: vscode.Uri;
  comparison: Comparison;
} {
  const data = JSON.parse(uri.query) as GraphiteURIEncodedData;
  return {
