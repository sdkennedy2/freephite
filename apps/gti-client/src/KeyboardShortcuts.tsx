...
type CommandMap<CommandName extends string> = Record<
  CommandName,
  CommandDefinition
>;
...
export function makeCommandDispatcher<CommandName extends string>(
  ...
  return [
    ({ children }) => (
      <Context.Provider value={commandDispatcher}>{children}</Context.Provider>
    ),
    useCommand,
    (command: CommandName) =>
      commandDispatcher.dispatchEvent(new Event(command)),
  ];
}
