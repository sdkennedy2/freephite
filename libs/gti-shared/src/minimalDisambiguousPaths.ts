...
/**
   ...
   */
export function minimalDisambiguousPaths(
  paths: string[],
  options: Options = {}
): string[] {
  const pathObjects: PathObject[] = paths.map((path) => {
    ...
    return {
      ...
      hadLeadingSeparator:
        ...
        (rootPrefix.length > 0 && path.startsWith(rootPrefix)),
    };
  });
  ...
  for (let currentDepth = 1; currentDepth <= maxDepth; currentDepth++) {
    ...
    for (const pathObject of pathObjectsToProcess) {
      const path = pathObject.parts
        .slice(0, currentDepth)
        .join(pathObject.separator);
  ...
  return pathObjects.map(
    ({ depth, parts, rootPrefix, separator, hadLeadingSeparator }) => {
      let resultPathParts = parts
        .slice(0, depth == null ? maxDepth : depth)
        .reverse();
